...
function normalizeCheckin(entry) {
  ...
  const label = [raw.label, raw.word, raw.text, raw.value]
    .find(value => typeof value === 'string' && value.trim());
  if (!label) return null;
  const time = [raw.time, raw.at, raw.timestamp]
    .find(value => typeof value === 'string' && value.trim()) || '';
...
function normalizeDayRecord(dayValue, dateStr) {
  ...
  const weeklyPractice = raw.weeklyPractice && typeof raw.weeklyPractice === 'object'
    ? raw.weeklyPractice
    : {};
  const eveningReflection = raw.eveningReflection && typeof raw.eveningReflection === 'object'
    ? raw.eveningReflection
    : {};
  const weeklyIntegration = raw.weeklyIntegration && typeof raw.weeklyIntegration === 'object'
    ? raw.weeklyIntegration
    : null;
  const checkinsSource = Array.isArray(raw.emotionalCheckins)
    ? raw.emotionalCheckins
    : (Array.isArray(raw.checkins) ? raw.checkins : []);

  return {
    ...raw,
    morning: raw.morning && typeof raw.morning === 'object' ? raw.morning : {},
    evening: raw.evening && typeof raw.evening === 'object' ? raw.evening : {},
    _ritualKeys: raw._ritualKeys && typeof raw._ritualKeys === 'object' ? raw._ritualKeys : undefined,
    ...
    weeklyPractice: {
      ...
      type: typeof weeklyPractice.type === 'string' && weeklyPractice.type.trim()
        ? weeklyPractice.type
        : practice.type
    },
